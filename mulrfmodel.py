    ...
    def compute_cost(self, gtree):
        ...
        for sNode in stree_lca_dict:
            for gNode in gtree_lca_dict:
                if stree_lca_dict[sNode] != None and gtree_lca_dict[gNode] != None and len(stree_lca_dict[sNode]) > 1 and len(gtree_lca_dict[gNode]) > 1 and sNode != streeCopy.root and len(stree_lca_dict[sNode]) == len(gtree_lca_dict[gNode]) and gNode != gtree.root:
                ...
                elif sNode.is_leaf() or sNode == streeCopy.root:
                    cost  = cost - 1
                    break
